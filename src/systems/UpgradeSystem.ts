...
interface StatUpgradeDefinition {
    name: string;
    description: string;
    icon: string;
    value: number;
...
export const StatUpgrades: Record<StatKey, StatUpgradeDefinition> = {
    moveSpeed: {
        name: 'Swift Feet',
...
export class UpgradeSystem {
    player: Player;
    statStacks: Record<string, number>;
     ...
     * @returns {Object[]}
     */
    generateOptions(count: number = 3): UpgradeOption[] {
        ...
        for (const [key, def] of Object.entries(StatUpgrades)) {
            ...
            if (this.statStacks[statKey] < def.maxStacks) {
                availableOptions.push({
                    type: 'stat',
                    stat: statKey,
                    value: def.value,
     ...
     */
    _weightedSample(items: UpgradeOption[], count: number): UpgradeOption[] {
        const result = [];
